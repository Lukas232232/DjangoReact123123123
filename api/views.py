    ...
    def post(self, request, format=None):
        ...
        if code:
            ...
            if len(roomResult) > 0:
                ...
                return Response({'message': 'Room joined successfully'},
...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code:
            ...
            if len(room) > 0:
                serializer = RoomSerializer(room[0]).data
                serializer[
                    'is_host'] = self.request.session.session_key == room[
                        0].host
...
class CreateRoomView(APIView):
    ...
    def get(self, request):
        # Проверям сущестует ли определенные куки
        if request.COOKIES.get('sessionid'):
            print("Куки существуют")
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if queryset.exists():
                ...
                return Response({'message': RoomSerializer(room).data},
                                status=status.HTTP_200_OK)
            else:
                room = Room(host=host,
